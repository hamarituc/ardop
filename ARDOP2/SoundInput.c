...
#define MAX_DATA_LENGTH	10 * 160 // I think! 16QAM.2500.100
...
// ARDOP 2 only hah one and two carrier FSK modes
...
int intSumCounts[10];			// number in above arrays
...
short intPhases[10][652] = {0};	// We will decode as soon as we have 4 or 8 depending on mode
								//	(but need one set per carrier)
								// 652 is 163 * 4 (16QAM 100 Baud)

short intMags[10][652] = {0};
...
int CorrectRawDataWithRS(UCHAR * bytRawData, UCHAR * bytCorrectedData, int intDataLen, int intRSLen, int bytFrameType, int Carrier)
{
	...
	if (CheckCRC16FrameType(bytRawData, intDataLen + 1, bytFrameType)) // No RS correction needed
	{
		...
		memcpy(bytCorrectedData, &bytRawData[1], bytRawData[0]);    
		WriteDebugLog(LOGDEBUG, "[CorrectRawDataWithRS] OK without RS, Len = %d", bytRawData[0]);
...
//		WriteDebugLog(LOGDEBUG, "RS Says OK after %d correction(s)", NErrors);
    ...
    if (FrameOK &&  CheckCRC16FrameType(bytRawData, intDataLen + 1, bytFrameType)) // RS correction successful 
	{
        ...
        WriteDebugLog(LOGDEBUG, "[CorrectRawDataWithRS] OK with RS %d corrections, Len = %d",  NErrors, bytRawData[0]);
...
short intPSKPhase_1[10], intPSKPhase_0[10];
			...
			if (IsDataFrame(intFrameType))
				if (intNumCar == 1)
					...
					WriteDebugLog(LOGDEBUG, "MEMARQ Flags %d %d %d %d %d %d %d %d %d %d",
						...
						CarrierOk[4], CarrierOk[5], CarrierOk[6], CarrierOk[7], CarrierOk[8], CarrierOk[9]);
		}
	}
	// Acquire Frame
