...
unsigned int dttLastGoodFrameTypeDecode;
extern int NErrors;
...
float dblPwrSNPower_dBPrior2 = 0;	// Try 3 sucessive good s/n figures
...
//	mix but don't filter - used for fine tune search

void MixNCO(short * intNewSamples, short * outsamples, int Length, float dblOffsetHz)
{
	int i;
	float dblNCOFreq;			// nominal NCO frequency
	float dblNCOPhase = 0;
	float dblNCOPhaseInc;  // was dblNCOFreq
	...
	for (i = 0; i < Length; i++)
	{
		outsamples[i] = (int)ceilf(intNewSamples[i] * cosf(dblNCOPhase));  // later may want a lower "cost" implementation of "Cos"
...
//	 2?m/N, corresponding to a cyclic frequency of mfs/N Hz (where fs is the signal sample rate).
...
//		dblAvgNoisePerBin = (dblMagWindow - (dblMag[i - 1] + dblMag[i] + dblMag[i + 1] + dblMag[i + 2] + dblMag[i + 3] + dblMag[i + 4] + dblMag[i + 5])) / (intStopBin - (intStartBin + 7));
		...
		dblAvgNoisePerBin = (dblMag[i - 3] + dblMag[i - 2] + dblMag[i + 6] + dblMag[i + 7]) / 2; // 2 as we compare to two tones
	...
	// Make sure three successive samples above squelch

	if (dblPwrSNdB > (2 * Squelch) 
		&& dblPwrSNPower_dBPrior > (2 * Squelch)
		&& dblPwrSNPower_dBPrior2 > (2 * Squelch))
	{	
		// Do the interpolation based on the two carriers at nominal 1475 and 1525Hz
		...
		if (((intIatMaxPeak - 99) >= intStartBin) && ((intIatMaxPeak - 103) <= intStopBin)) // check to ensure no index errors
		{
			...
			WriteDebugLog(LOGDEBUG, "Tone Ratio Left %1.1f Right %1.1f Ratio %1.3f SNdB %1.1f Prior %1.1f Prior2 %1.1f Offset %1.3f",
				Left, Right, Left / Right, dblPwrSNdB, dblPwrSNPower_dBPrior, dblPwrSNPower_dBPrior2, dblOffset);
	       ...
	       if (Left < Right / 4 || Right < Left /4)
		   {
			...
			{
				short outsamples[960];
				float Delta, Mag, MaxMag = 0.0f, Real, Imag;
				float DeltaAtMax;
				...
				*intSN = round(dblPwrSNdB - 20.8); // 20.8dB accomodates ratio of 3Kz BW: (effective Blackman Window bandwidth  of ~25 Hz) 
				...
				// Ardop3 seems to need very accurate tuning, so need a way to improve it

				// Averaging could help, but probably not enough. Want to get within .2 to .3 Hz.

				// I'm going to try repeatedly mixing with a range of say +-3 and
				// looking for peak at exactly 1475 (Or 15250.

				// May use far too much CPU, especially on Teensy

				// ?? Could we do successive approximation to reduce steps ??

				// We must have 960 or more samples to get here.

				// ? do we need to do post mix filter ??

				Delta = -3.0f;
				/*
				printtick("Start Fine Tune");

				while (Delta < 3.0f)
				{
					MixNCO(intNewSamples, outsamples, 960, dblOffset + Delta);

					// Get magnitude at 1475 (Bin 118);

					GoertzelRealImagHamming960(outsamples, 0, 960, 118, &Real, &Imag);
					Mag = (Real * Real) + (Imag * Imag);	 //Mag in units of power (V^2)
	
				   	WriteDebugLog(LOGDEBUG, "Fine Tune Delta %f Mag %f", Delta, Mag);
				...
				WriteDebugLog(LOGDEBUG, "Fine Tune Delta %f Final Offset %f", DeltaAtMax, dblOffset - DeltaAtMax);

				printtick("End Fine Tune");
*/
				return True;
			}
			else
	...
	dblPwrSNPower_dBPrior2 = dblPwrSNPower_dBPrior;
	dblPwrSNPower_dBPrior = dblPwrSNdB;
...
BOOL AFSFirstTime = TRUE;		// Init Acquire Frame Sync Flag
...
// Need to maintain state across sample buffers
...
int AcquireFrameSyncRSBAvg()
{
	...
	int intAvailableSymbols = (intFilteredMixedSamplesLength - intMFSReadPtr) / 240;
	int intAvailableSamples = intFilteredMixedSamplesLength - intMFSReadPtr;
	...
	if (AFSFirstTime)
	{
		...
		GoertzelRealImagHann120(intFilteredMixedSamples, intMFSReadPtr, 240, 30, &dblReal, &dblImag); // Carrier at 1500 Hz nominal Positioning with no cyclic prefix
		dblPhaseSym1 = atan2f(dblImag, dblReal);
		intMFSReadPtr += 240;	// advance one symbol
		GoertzelRealImagHann120(intFilteredMixedSamples, intMFSReadPtr, 240, 30, &dblReal, &dblImag); // Carrier at 1500 Hz nominal Positioning with no cyclic prefix
		dblPhaseSym2 = atan2f(dblImag, dblReal);
		intMFSReadPtr += 240;	// advance one symbol
...
// not used			dttLastLeaderSync = Now;
			...
			WriteDebugLog(LOGDEBUG, "Got Frame Sync. Remote Leader %d", intLeaderRcvdMs);

			dblPwrSNPower_dBPrior2 = dblPwrSNPower_dBPrior = -1000;  // Reset the prior Leader power to small value to insure minimum of two symbol passes on next leader detect. 
			return TRUE;	 // pointer is pointing to first 4FSK data symbol. (first symbol of frame type)
		}
...
int EstimateSN()
{
	...
	// Don't know why, but first two mags are always way off

	for (i = 2; i < intPhasesLen; i++)
	{
...
void CorrectPhaseForTuningOffset(short * intPhase, int intPhaseLength, char * strMod)
{
	...
	int intTest;
	int i;
	int intOffset, intAvgOffset = 0, intAvgOffsetBeginning = 0, intAvgOffsetEnd = 0;
...
void DemodViterbiPSK()
{
	...
	while (State == AcquireFrame)
	{
		...
		if (intNumCar > 4)
		{
			...
			CorrectPhaseForTuningOffset(&intPhases[8][0], intPhasesLen, strMod);
			CorrectPhaseForTuningOffset(&intPhases[9][0], intPhasesLen, strMod);
		}
